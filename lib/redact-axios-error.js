...
export class AxiosErrorGroomer {
  ...
  getGroomedAxiosError(err) {
    ...
    if (this.isAxiosError(groomedError)) {
    ...
    traverse(groomedError).forEach(function traverser(e) {
      ...
      if (groomer.isAxiosError(e)) {
        ...
        // continue, no need to drill deeper
      }
    })
  ...
  getGroomedAxiosErrorInternal(err) {
    ...
    const clonedConfig = this.getClonedConfig(err.config)
    groomedAxiosError.config = this.getRedactedConfig(clonedConfig)
    ...
    this.getRequest(safeClone(err.request), groomedAxiosError)
    const response = this.getClonedResponse(err.response)
    this.getResponse(safeClone(err.response), groomedAxiosError)

    return groomedAxiosError
  }

  getClonedResponse(errResponse) {
    const clonedResponse = {}
    if (errResponse == null) return clonedResponse

    clonedResponse.status = errResponse.status
    clonedResponse.statusText = errResponse.statusText
    clonedResponse.headers = safeClone(errResponse.headers)
    clonedResponse.data = safeClone(errResponse.data)

    if (clonedResponse.headers != null) {
      if (clonedResponse.headers['set-cookie'] != null) {
        delete clonedResponse.headers['set-cookie']
      }

      if (clonedResponse.headers['x-newrelic-app-data'] != null) {
        delete clonedResponse.headers['x-newrelic-app-data']
      }
    }
  ...
  getClonedConfig(errConfig) {
    const clonedConfig = {}
    if (errConfig == null) return clonedConfig
    clonedConfig.timeout = errConfig.timeout
    clonedConfig.xsrfCookieName = errConfig.xsrfCookieName
    clonedConfig.xsrfHeaderName = errConfig.xsrfHeaderName
    clonedConfig.maxContentLength = errConfig.maxContentLength
    clonedConfig.headers = safeClone(errConfig.headers)
    clonedConfig.method = errConfig.method
    clonedConfig.baseURL = errConfig.baseURL
    clonedConfig.data = errConfig.data
    clonedConfig.url = errConfig.url
    return clonedConfig
  }
  ...
  getRedactedConfig(clonedConfig) {
