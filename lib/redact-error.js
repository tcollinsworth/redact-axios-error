...
export class AxiosErrorGroomer {
   ...
   *
   * @param {Object} e The error to test.
   * @returns {boolean} True if axios error.
   */
  // eslint-disable-next-line class-methods-use-this
  isAxiosError(e) {
    return e instanceof Error && e.config != null && e.request != null
  }

  // TODO recursively scan Error properties and redact any which are axios Errors, max depth 5
  ...
  getGroomedAxiosError(e) {
    if (!this.isAxiosError(e)) return e

    const groomedAxiosError = new Error('e.message')
    groomedAxiosError.stack = e.stack

    groomedAxiosError.config = this.getRedactedConfig(e.config)

    this.redactConfigData(groomedAxiosError.config)

    groomedAxiosError.errno = e.errno
    groomedAxiosError.code = e.code
    groomedAxiosError.syscall = e.syscall
    groomedAxiosError.address = e.address
    groomedAxiosError.port = e.port

    this.getRequest(e.request, groomedAxiosError)
    this.getResponse(e.response, groomedAxiosError)
...
//   },
//   "method": "get",
//   "baseURL": "http://user:pass@localhost:3000",
//   "url": "http://user:pass@localhost:3000/error"
// }
